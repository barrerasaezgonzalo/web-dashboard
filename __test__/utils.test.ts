...
import { Financial, FinancialHistory, Indicator, Task, PromptData } from "@/types";
...
describe("Funciones de Utilidades", () => {
    ...
    describe("parsePromptResponse", () => {
      ...
      `;
        ...
        Gracias.
      `;
            expect(parsePromptResponse(raw)).toEqual({ name: "Test", value: 123, nested: ["a", "b"] });
        });
      ...
      `;
            expect(parsePromptResponse(raw)).toEqual({ title: "Ejemplo", data: [1, 2, 3] });
        });
    ...
    describe("getIndicators", () => {
        ...
        test("debe mapear el objeto Financial a un array de Indicator", () => {
            ...
            expect(indicators[0]).toEqual({ label: "Dólar", value: "$950.5", key: "dolar" });
            expect(indicators[1]).toEqual({ label: "UTM", value: "$65.182", key: "utm" });
            expect(indicators[2]).toEqual({ label: "BTC", value: "$60.000.000", key: "btc" });
            expect(indicators[3]).toEqual({ label: "ETH", value: "$3.000.000", key: "eth" });
        });
    });
    ...
    describe("mapSparklineData", () => {
        const history: FinancialHistory[] = [
            { created_at: "2023-10-01", dolar: 900, utm: 60000, btc: 50000000, eth: 2000000 },
            { created_at: "2023-10-02", dolar: 910, utm: 61000, btc: 55000000, eth: 2500000 },
        ];
    ...
    describe("getTrend", () => {
        const historyUp: FinancialHistory[] = [
            { created_at: "D1", dolar: 900 } as FinancialHistory,
            { created_at: "D2", dolar: 910 } as FinancialHistory,
        ];
        const historyDown: FinancialHistory[] = [
            { created_at: "D1", dolar: 910 } as FinancialHistory,
            { created_at: "D2", dolar: 900 } as FinancialHistory,
        ];
        const historySame: FinancialHistory[] = [
            { created_at: "D1", dolar: 900 } as FinancialHistory,
            { created_at: "D2", dolar: 900 } as FinancialHistory,
        ];
        const historySingle: FinancialHistory[] = [
            { created_at: "D1", dolar: 900 } as FinancialHistory,
        ];
        const historyEmpty: FinancialHistory[] = [];
        ...
        test("debe devolver 'same' si los dos últimos valores son iguales", () => {
            expect(getTrend(historySame, "dolar")).toBe("same");
        });
        ...
        test("debe funcionar con otras claves (e.g., 'btc')", () => {
            const btcHistoryUp: FinancialHistory[] = [
                { btc: 50000000 } as FinancialHistory,
                { btc: 55000000 } as FinancialHistory,
            ];
            expect(getTrend(btcHistoryUp, "btc")).toBe("up");
        });
    });
