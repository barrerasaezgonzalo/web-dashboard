...
export const NewsProvider: React.FC<NewsProviderProps> = ({ children }) => {
    ...
    const getNews = async () => {
        ...
        try {
            ...
            if (typeof window !== "undefined") {
                ...
                localStorage.setItem("newsCacheTime", (Date.now() + 12 * 60 * 60 * 1000).toString()); // válida 12 horas
            }
    ...
    const bloquearNews12Horas = () => {
        if (typeof window !== "undefined") {
            localStorage.setItem("newsCacheTime", (Date.now() + 12 * 60 * 60 * 1000).toString()); // bloquea 12h
            setNewsCache(null);
            getNews();
        }
    };
